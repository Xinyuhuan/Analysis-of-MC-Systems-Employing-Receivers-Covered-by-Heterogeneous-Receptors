%% Analytical expressions for ploting the bit error rate when the transmitter is MF-based
...
etat=0:1:150;
eta=1:150;
berf=zeros(length(n),length(eta));
zs=zeros(1,length(eta));
for i=1:length(n)
    a=abs(dec2bin(0:(2^n(i)-1), n(i)))-48;
    a(1,:)=[];
    [col,~]=size(a);
    brf=zeros(col,length(eta));
    for j=1:col
        chi=sum(a(j,:).*b(i:10));
        pr=chi.^etat.*exp(-chi)./factorial(etat);
        cdf_pr=zeros(1,length(eta));
        for p=1:length(eta)
            cdf_pr(p)=sum(pr(1:p));
        end
        if a(j,end)==1
